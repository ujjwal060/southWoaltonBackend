...
const reservationHistoryByUserId = async (req, res, next) => {
  const { userId } = req.params;
  ...
  try {
    ...
    const startIndex = (page - 1) * limit;
    const paginatedReservations = reservations.slice(startIndex, startIndex + parseInt(limit));

    // Fetch vehicle details manually
    const enrichedReservations = await Promise.all(
      paginatedReservations.map(async (reservation) => {
        const vehicle = await Vehicle.findOne({ _id: reservation.vehicleId }); // still works if it's a valid ObjectId string
        return {
    ...
    return next(createError(500, "Error fetching reservation history"));
  }
};
